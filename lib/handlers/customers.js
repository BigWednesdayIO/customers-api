...
const customerSchema = baseCustomerSchema.concat(joi.object({
  ...
  address: joi.object({
    name: joi.string().description('Contact name for the customer address'),
    company: joi.string().description('Company name for the customer address'),
    line_1: joi.string().description('Line one of the customer address'),
    ...
    postcode: joi.string().description('Postcode of the customer address')
  }).meta({className: 'CustomerAddress'})
}))
